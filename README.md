...
- **资讯流** — 面向大众用户的真实论文 feeds 流：优先从 Google Scholar 发现环境健康领域论文，再用 OpenAlex 补全摘要、DOI、期刊、引用数等元数据，最后生成中文通俗摘要；不再使用本地虚构资讯
- **科研工作台** — 上传 CSV 后自动完成字段合规审查、数据体检、多模型对比与最优模型推荐
- **工作台结构重构** — 页面已按“数据准备 -> 模型分析 -> 报告产出 -> 协作留痕”四阶段重排，降低信息堆叠带来的混乱感
- **资讯 <-> 科研联动** — 论文可一键导入科研工作台，自动生成研究问题、数据需求、协作任务；工作台结论只生成编辑草稿，不再直接混入大众资讯 feed
...
| POST | `/api/news/publish-workbench` | 已限制直发：大众资讯流只允许真实论文解读，工作台结论需作为编辑草稿或论文详情补充 |
...
- 在“专业工作台”里可直接点击四组示例数据，预览：
  ...
  - 高风险隐私/版权拦截与 AI 脱敏方案
- 在“资讯流”里可进一步预览：
  - 按分类抓取 Google Scholar / OpenAlex 真实论文并 30 分钟自动刷新
  - 仅看开放获取 OA / 高被引 / 近一年论文
  - 将论文导入科研工作台，继续做研究假设拆解、模型验证和协作分工
  - 查看“工作台结果为什么不能直接混入资讯流”的风险判断与编辑草稿
