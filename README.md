...
- **资讯流** — 基于 OpenAlex 真实论文抓取的环境健康资讯流，支持分类筛选、搜索、点赞评论，以及 OA / 高被引 / 近一年过滤
- **科研工作台** — 上传 CSV 后自动完成字段合规审查、数据体检、多模型对比与最优模型推荐
- **资讯 <-> 科研联动** — 论文可一键导入科研工作台，自动生成研究问题、数据需求、协作任务，并把分析结果回流成公众版摘要
...
| `ENABLE_NEWS_AI_SUMMARY` | 可选；设为 `true` 时，资讯侧对真实论文使用 OpenAI 做更细的中文通俗化解读 |
| `APP_URL` | 应用部署地址 |
